...
{
	[color set];
	[words drawAtPoint:point withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12]}];
}
...
-(void)drawXYAxixTitleWithColor:(UIColor *)color
{
	...
	[lb_xaxisTitle setTextAlignment:NSTextAlignmentCenter];
	...
	[lb_yaxisTitle setTextAlignment:NSTextAlignmentCenter];
	...
	[lb_graphicTitle setTextAlignment:NSTextAlignmentCenter];
...
-(int)getXDaysFromLines:(NSArray *)lines minDate:(NSDate **)minDate;
{
	...
	{
		ECGraphLine *line = lines[i];
		NSArray *points = line.points;
		for (int j = 0; j < [points count]; ++j)
		{
			ECGraphPoint *graphicPoint = points[j];
...
-(id)initWithFrame:(CGRect)frame withContext:(CGContextRef)context isPortrait:(BOOL)isPortrait
{
	if (self = [super init]) 
	{
		...
		_defaultColors = @[[UIColor colorWithRed:1 green:0 blue:0 alpha:1],
						  ...
						  [UIColor colorWithRed:0 green:0 blue:153/255.0  alpha:1],[UIColor colorWithRed:220/255.0 green:20/255.0  blue:60/255.0 alpha:1]];
...
-(void)drawCurveWithLines:(NSArray *)lines lineWidth:(int)lineWidth color:(UIColor *)color
{
	...
	ECGraphLine *line = lines[0];
	if(line.isXDate)
	{
		_minDate = [[NSDate alloc] init];
        NSDate *md = _minDate;
		int durationDays = [self getXDaysFromLines:lines minDate:&md];
        _minDate = md;
		if (durationDays == 1) {
	...
	for (int i = 0; i < [lines count]; ++i) 
	{
		...
		{
			color = _defaultColors[line.defaultColor];
		}
		else {
			color = line.color;
		}
...
-(void)drawLineWithPoints:(NSArray *)points lineWidth:(int)lineWidth color:(UIColor *)color
{
	...
	{
		durationDays1 = [self getDaysFrom:_minDate To:ECCAST(ECGraphPoint,points[i]).xDateValue];
		durationDays2 = [self getDaysFrom:_minDate To:ECCAST(ECGraphPoint,points[i+1]).xDateValue];
		
		
		[self drawLineFromPoint:CGPointMake(_xaxisStart.x +_xPointsCount*(durationDays1 - 1)*_xSpacingLength,_xaxisStart.y - _ySpacingScale*ECCAST(ECGraphPoint,points[i]).yValue) 
						toPoint:CGPointMake(_xaxisStart.x +_xPointsCount*(durationDays2 - 1)*_xSpacingLength,_xaxisStart.y - _ySpacingScale*ECCAST(ECGraphPoint,points[i+1]).yValue) lineWidth:2 color:color];
		switch (_pointType) {
			case ECGraphPointTypeCircle:
				[self drawCircleAtPoint:CGPointMake(_xaxisStart.x +_xPointsCount*(durationDays1 - 1)*_xSpacingLength,_xaxisStart.y - _ySpacingScale*ECCAST(ECGraphPoint,points[i]).yValue)  
						   withDiameter:5.0 fillColor:color stockeColor:color];
				break;
			case ECGraphPointTypeTriangle:
				[self drawTriangleAtPoint: CGPointMake(_xaxisStart.x +_xPointsCount*(durationDays1 - 1)*_xSpacingLength,_xaxisStart.y - _ySpacingScale*ECCAST(ECGraphPoint,points[i]).yValue)
							 withDiameter:5.0 fillColor:color stockeColor:color rotate:2.0];
				break;
			case ECGraphPointTypeSquare:
				[self drawSquareAtPoint: CGPointMake(_xaxisStart.x +_xPointsCount*(durationDays1 - 1)*_xSpacingLength,_xaxisStart.y - _ySpacingScale*ECCAST(ECGraphPoint,points[i]).yValue)
							 withDiameter:5.0 fillColor:color stockeColor:color rotate:2.0];
				break;
			default:
				break;
	...
	//draw the last point
	durationDays1 = [self getDaysFrom:_minDate To:ECCAST(ECGraphPoint,points[[points count] - 1]).xDateValue];
	switch (_pointType) {
		case ECGraphPointTypeCircle:
			[self drawCircleAtPoint:CGPointMake(_xaxisStart.x +_xPointsCount*(durationDays1 - 1)*_xSpacingLength,_xaxisStart.y - _ySpacingScale*ECCAST(ECGraphPoint,points[[points count] - 1]).yValue)  
					   withDiameter:5.0 fillColor:color stockeColor:color];
			break;
		case ECGraphPointTypeTriangle:
			[self drawTriangleAtPoint: CGPointMake(_xaxisStart.x +_xPointsCount*(durationDays1 - 1)*_xSpacingLength,_xaxisStart.y - _ySpacingScale*ECCAST(ECGraphPoint,points[[points count] - 1]).yValue)
						 withDiameter:5.0 fillColor:color stockeColor:color rotate:2.0];
			break;
		case ECGraphPointTypeSquare:
			[self drawSquareAtPoint: CGPointMake(_xaxisStart.x +_xPointsCount*(durationDays1 - 1)*_xSpacingLength,_xaxisStart.y - _ySpacingScale*ECCAST(ECGraphPoint,points[[points count] - 1]).yValue)
					   withDiameter:5.0 fillColor:color stockeColor:color rotate:2.0];
			break;
		default:
			break;
...
-(void)drawHistogramWithItems:(NSArray *)items lineWidth:(int)lineWidth color:(UIColor *)color
{
	...
	for (int i = 0; i < itemsCount; ++i) 
	{
		...
		{
			color = _defaultColors[item.defaultColor];
		}
		else {
			color = item.color;
		}
...
-(void)drawHistogramWithItem:(ECGraphItem *)item index:(int)index color:(UIColor *)color
{
	...
	[percentage drawAtPoint:CGPointMake(_histogramStartX,_xaxisStart.y - _ySpacingScale*item.yValue - 15) withAttributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:10]}];
