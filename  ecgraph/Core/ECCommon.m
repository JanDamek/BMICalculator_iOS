...
+ (NSString *)sBetweenW1:(NSString *)w1 W2:(NSString *)w2 OfS:(NSString *)aString;
{
	...
	{
		int location1 = (int)range1.location ;
...
+ (void)vStringCompareOfA:(NSMutableArray *)aArray
{
	for(int i=1;i<[aArray count];++i)
	{
		...
		{
			if([aArray[j] compare:aArray[j+1]] == NSOrderedDescending)
			{
				NSString *temp = [NSString stringWithString:aArray[j]];
				aArray[j] = aArray[j+1];
				aArray[j+1] = temp;
			}
		}
...
+ (NSString *)sWordMiddleToAllNumberOfS:(NSString *)aString SplitS:(NSString *)splitString
{
	...
	NSString *year = datePartArray[2];
	NSString *month = [self sMonthToNumberOfS:datePartArray[1]];
	NSString *day = datePartArray[0];
...
{
	NSArray *datePartArray = [aString componentsSeparatedByString:splitString];
	NSString *year = datePartArray[0];
	NSString *month = [self sNumberToMonthOfS:datePartArray[1]];
	NSString *day = datePartArray[2];
...
+(NSMutableArray  *)arrDistinct:(NSMutableArray  *)arr
{
	...
	{
		if (cur_number != [arr[i] intValue])
		{
			[temp addObject:arr[i]];
		}
		cur_number = [arr[i] intValue];
	}
	return temp;
}
...
+(void)vMerge:(NSMutableArray *)arr left:(int)left center:(int)center right:(int)right type:(NSString *)type
{
	...
	while (left1<= right1 && left2 <= right2) 
	{
		if ([type isEqual:@"tag"]) 
		{
			if (((UIView *)arr[left1]).tag < ((UIView *)arr[left2]).tag) 
			{
				[temp addObject:arr[left1++]];
			}
			else
			{
				[temp addObject:arr[left2++]];
			}

		}
		else 
		{
			if ([arr[left1] intValue] < [arr[left2] intValue])
			{
				[temp addObject:arr[left1++]];
			}
			else
